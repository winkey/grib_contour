...
wwwdisk="/home/rush/publichtml/weather/kml/kml/ecmwf"
...
then
	...
	then
		...
		nice -n 10 grib_contour "${paths[@]}" -i 20 -s ${level}hgt -k "$kml" -z "$zip"
...
then
	...
	then
		...
		nice -n 10 grib_contour "${paths[@]}" -i 200 -s pmsl -k "$kml" -z "$zip"
